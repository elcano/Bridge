...
The HL is connected to an SD card through SPI (MOSI, MISO, SCK and CS). If jumpers are inserted in J@2, the SPI pins are also connected to the Router. This lets the router control the SD card when the HL does not use it. To let the Router use the SD card, the CS from HL needs to be routed to a digital pin on the router. The schematic shows this as TP15, but the test points on the schematic do not match those on the boards.
